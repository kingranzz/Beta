...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Chucky says type your WhatsApp number 👑\nFor example: +916909137213 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: marx`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: marxalone `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @marxalone `))
